...
- **주문 생성:**
    - [x] 여러 개의 상품으로 주문을 생성할 수 있다.
    - [x] 주문 생성 시 주문 상태는 '주문 접수'가 된다.
    - [x] 주문 생성 시 주문 시간과 주문 번호가 올바르게 기록된다.
- **주문 목록 관리:**
    - [x] 주문 목록에 상품을 추가할 수 있다.
    - [x] 주문 목록에서 특정 상품을 삭제할 수 있다.
    - [x] 주문 목록의 모든 상품을 삭제할 수 있다.
    - [x] 주문 목록에 담긴 상품의 개수를 변경할 수 있다.
    - [x] 상품 개수를 0개 이하로 변경하려고 하면 예외가 발생한다.
- **금액 계산:**
    - [x] 주문 목록에 있는 모든 상품의 총 금액을 올바르게 계산한다.
- **주문 상태 변경:** (새로 추가된 시나리오)
...
### Phase 3: 주문 기능 구현 (TDD) - 주문 목록 관리 및 금액 계산 (2026-01-08)
- [x] `OrderCart`, `OrderCartItem` 클래스 생성 및 관련 로직 리팩토링
- [x] 주문 목록 관리(추가, 삭제, 전체 삭제, 수량 변경) 서비스 로직 구현
- [x] 주문 목록 총 금액 계산 로직 구현
- [x] 관련 기능에 대한 단위 테스트 및 통합 테스트 코드 작성
